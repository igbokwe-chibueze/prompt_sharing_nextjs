// app/api/incrementprofileClick/[id].js
...
export const POST = async (request, { params }) => {
    ...
    try {
        ...
        const updatedEntity = await Model.findByIdAndUpdate(
            entity,
            { $inc: { profileClickCount: 1 } }, // Use the $inc operator to increment the profileClickCount by 1
            { new: true } // Return the updated document after the update operation
        );

        return new Response(JSON.stringify({ updatedEntity }), { status: 200 });
    } catch (error) {
        return new Response("Error updating profileClickCount", { status: 500 });
    }
}
